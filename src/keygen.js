...
const GITPWNED_LICENSE = process.env.GITPWNED_LICENSE;
...
// validateKey handles the validation and/or activation of a GITPWNED_LICENSE
...
async function ValidateKey(eventJSON) {
  ...
  switch (validateKeyResponse.meta.constant) {
    ...
    case "TOO_MANY_MACHINES":
      core.error(
        `🛑 Cannot use GITPWNED-action on this repo. Your license key has already reached its limit of [${validateKeyResponse.data.attributes.maxMachines}] repos. Go to GITPWNED.io to upgrade your license to enable additional repos.`
      );
      process.exit(1);
    ...
    case "NO_MACHINE":
      ...
      var activationRequestOptions = {
        ...
        headers: {
          "Content-Type": "application/vnd.api+json",
          Accept: "application/vnd.api+json",
          Authorization: `License ${GITPWNED_LICENSE}`,
        },
      };
